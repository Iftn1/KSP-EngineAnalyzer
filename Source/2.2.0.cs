...
namespace KSP_EngineAnalyzer
{
    ...
    public class EngineAnalyzerWindow : MonoBehaviour
    {
        ...
        private List<CelestialBody> targetBodies = new List<CelestialBody>();
        ...
        {
            if (targetBodies.Count > 0) return;
            try
            {
        ...
        private void LoadSettings()
        {
            ...
            selectedBodyIndex = PlayerPrefs.GetInt("EA_selectedBodyIndex", 0);
        }
        ...
        private void SaveSettings()
        {
            ...
            PlayerPrefs.SetInt("EA_selectedBodyIndex", selectedBodyIndex);
            PlayerPrefs.Save();
        }
        ...
        {
            if (targetBodies.Count > 0 && selectedBodyIndex >= 0 && selectedBodyIndex < targetBodies.Count)
                return targetBodies[selectedBodyIndex];
            try
            {
            ...
            try
            {
                var home = FlightGlobals.Bodies?.FirstOrDefault(b => b != null && b.isHomeWorld);
                if (home != null) return home;
            }
            catch { }
            return FlightGlobals.Bodies?.FirstOrDefault(b => b != null) ?? null;
        }
        ...
        private void RefreshData()
        {
            try
            {
                ...
                foreach (AvailablePart ap in PartLoader.LoadedPartsList)
                {
                    ...
                    // Primary: try runtime ModuleEngineConfigs.configs via reflection (works for all engine types, not just SRBs)
                        ...
                        if (allConfigs == null)
                        {
                            foreach (var f in mecType.GetFields(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic))
                            {
                                ...
                                if ((isConfigArray || isConfigList) && f.Name.ToLower().Contains("config"))
                                {
                                    ...
                                    if (allConfigs != null) break;
                                }
                            }
                        }
                    }

                    // Fallback: try partConfig CONFIG nodes
                    if (allConfigs == null && mecNode != null && mecNode.HasNode("CONFIG"))
                    {
                    ...
                    if (allConfigs != null && allConfigs.Length > 0)
                    {
                        foreach (ConfigNode cfg in allConfigs)
                        {
                            ...
                            // Extract config-specific fuel mixture (density + fuel types) from PROPELLANT nodes
                            float cfgMixDensity = baseMixDensity;
                            List<string> cfgFuels = new List<string>(baseFuels);
                            if (cfg.HasNode("PROPELLANT"))
                            {
                                var cfgProps = cfg.GetNodes("PROPELLANT");
                                float cfgTotalRatio = 0f;
                                foreach (ConfigNode pn in cfgProps)
                                {
                                    float r = 0f;
                                    if (pn.HasValue("ratio")) float.TryParse(pn.GetValue("ratio"), out r);
                                    cfgTotalRatio += r;
                                }
                                if (cfgTotalRatio > 0)
                                {
                                    ...
                                    foreach (ConfigNode pn in cfgProps)
                                    {
                                        string pnName = pn.GetValue("name") ?? "";
                                        float ratio = 0f;
                                        if (pn.HasValue("ratio")) float.TryParse(pn.GetValue("ratio"), out ratio);
                            ...
                            var config = CalculateConfig(cfgName, cfgT, cfgIV, cfgIA, cfgMixDensity, cfgFuels, baseUllage, baseIgnitions, configBurnTimeStr, ap, engineCount, isHP, isSRB ? cfgAvgThrust : 0f, isSRB ? solidFuelMass : -1f);
            ...
            catch { allGroupsCache.Clear(); filteredGroups.Clear(); }
        }
        ...
        {
            const float g0Standard = 9.80665f;

            CelestialBody currentBody;
            float currentG, bodyAtm;
            try
            {
                currentBody = GetCurrentBody();
                currentG = (currentBody != null) ? (float)(currentBody.GeeASL * g0Standard) : g0Standard;
                bodyAtm = (currentBody != null && currentBody.atmosphere) ? (float)(currentBody.GetPressure(0.0) / 101.325) : 1.0f;
            }
            catch
            {
                currentG = g0Standard;
            ...
            {
                if (currentBody != null && currentBody.atmosphere && engineModule != null && engineModule.atmosphereCurve != null)
                {
                    float ispAtBody = engineModule.atmosphereCurve.Evaluate(bodyAtm);
                    isp = ispAtBody;
                    thrust = effectiveT * (isp / Math.Max(IV, 0.01f)) * cnt;
                }
                else
                {
                    isp = IV;
            ...
            if (preFuelMass >= 0f)
            {
                ...
                if (dry > 0) dV = isp * g0Standard * Mathf.Log(wet / dry);
                twr = thrust / (wet * currentG);
            }
            ...
            {
                float R = Mathf.Exp(targetDV / (isp * g0Standard));
                wet = dry * R;
            ...
            return new EngineConfig { configName = name, deltaV = dV, twr = twr, ispVac = IV, ispASL = isVacMode ? IV : isp, price = ap.cost * cnt, displayWetMass = wet, displayVolume = vol, fuelInfo = string.Join("/", f), ullageInfo = ull, ignitionsDisplay = ign, burnTimeDisplay = bt, meetsSmartCriteria = meet, needsHighPressure = isHP, maxThrust = T * cnt };
        }
        ...
        private void OnGUI()
        {
            ...
            if (showBodyDropdown)
            {
                bodyDropdownRect = GUILayout.Window(777, bodyDropdownRect, DrawBodyDropdown, L("#engineAnalyzer_TargetPlanet"));
        ...
        private void DrawWindow(int id)
        {
            ...
            try
            {
                if (targetBodies.Count > 1 && selectedBodyIndex >= 0 && selectedBodyIndex < targetBodies.Count)
                {
                    GUILayout.BeginHorizontal();
                    GUILayout.Label(L("#engineAnalyzer_TargetPlanet"), GUILayout.Width(80));

                    string currentName = targetBodies[selectedBodyIndex]?.bodyName ?? "Kerbin";
                    string btnText = $"▼ {currentName} ▼";

                    if (GUILayout.Button(btnText, GUILayout.MinWidth(130)))
                    {
                        showBodyDropdown = true;
                        _bodyDropdownScroll = Vector2.zero;
                        bodyDropdownRect.x = windowRect.x + windowRect.width + 10f;
                        bodyDropdownRect.y = windowRect.y;
                    }
                    GUILayout.FlexibleSpace();
                    GUILayout.EndHorizontal();
                }
            }
            catch { selectedBodyIndex = 0; targetBodies.Clear(); showBodyDropdown = false; }
        ...
        private void DrawBodyDropdown(int windowID)
        {
            float dropWidth = 200f;
            float itemHeight = 22f;
            int visibleCount = Math.Min(targetBodies.Count, 12);
            float dropHeight = visibleCount * itemHeight + 8f;

            GUILayout.BeginVertical(GUILayout.Width(dropWidth));
            _bodyDropdownScroll = GUILayout.BeginScrollView(_bodyDropdownScroll, GUILayout.Width(dropWidth), GUILayout.Height(dropHeight));

            for (int i = 0; i < targetBodies.Count; i++)
            {
                string bodyName = targetBodies[i]?.bodyName ?? "???";
                bool isSelected = (i == selectedBodyIndex);
                string displayName = isSelected
                    ? $"<color=lime><b>▶ {bodyName}</b></color>"
                    : $"  {bodyName}";

                if (GUILayout.Button(displayName, "label", GUILayout.Height(itemHeight)))
                {
        ...
        public void OnHotLoad(MonoBehaviour old)
        {
            ...
            if (oldWindow != null)
            {
                ...
                this.selectedBodyIndex = oldWindow.selectedBodyIndex;
                this.targetBodies = oldWindow.targetBodies;
